    ...
    return (
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-stone-900/90 backdrop-blur-md">
        <div className="bg-white rounded-3xl p-12 text-center shadow-2xl">
          ...
          <h3 className="text-xl font-bold font-jp mb-2">{t.modal.successTitle}</h3>
          <p className="text-stone-400 text-sm">{t.modal.successDesc}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-stone-900/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <form className="p-8 space-y-6" onSubmit={handleSubmit}>
          <div>
            <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">{t.modal.nameLbl}</label>
            <input required type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 transition-all" placeholder={t.modal.namePlaceholder} />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">{t.modal.phoneLbl}</label>
            <input required type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 transition-all" placeholder={t.modal.phonePlaceholder} />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">{t.modal.regionLbl}</label>
            <input type="text" value={region} onChange={e => setRegion(e.target.value)} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 transition-all" placeholder={t.modal.regionPlaceholder} />
          </div>
          ...
          <p className="text-center text-[9px] text-stone-400 uppercase tracking-widest">{t.modal.note}</p>
        </form>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-white font-sans selection:bg-red-100 selection:text-red-900">
      ...
      <header className="relative pt-48 pb-32 overflow-hidden bg-stone-900 min-h-[85vh] flex items-center">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          ...
          <h1 className="text-6xl md:text-8xl font-black text-white mb-8 font-jp leading-tight tracking-tighter">
            {t.hero.h1.line1}<br/><span className="text-red-600 underline decoration-stone-700 underline-offset-8">{t.hero.h1.highlight}</span>{t.hero.h1.line2}
          </h1>
          <p className="text-xl md:text-2xl text-stone-200 font-light tracking-widest max-w-3xl mx-auto uppercase mb-12">
            {t.hero.subtitle}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={() => scrollToSection('courses')} className="bg-red-600 hover:bg-red-700 text-white px-10 py-5 rounded-md font-bold text-sm tracking-widest uppercase transition-all shadow-2xl active:scale-95">{t.hero.ctaCourses}</button>
            <button onClick={() => scrollToSection('exhibition')} className="bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border border-white/30 px-10 py-5 rounded-md font-bold text-sm tracking-widest uppercase transition-all active:scale-95">{t.hero.ctaExhibition}</button>
          </div>
        </div>
      </header>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div className="relative">
              ...
              <div className="absolute -bottom-10 -right-10 bg-white p-10 shadow-xl rounded-xl border border-stone-100 hidden md:block max-w-xs">
                <p className="text-stone-900 font-jp font-bold text-xl mb-4 italic leading-relaxed">"{t.philosophy.quote}"</p>
            ...
            <div>
              ...
              <div className="space-y-10">
                {t.philosophy.pillars.map((pillar) => (
                  <div key={pillar.char} className="flex gap-6">
                    <div className="w-12 h-12 bg-red-50 text-red-600 flex items-center justify-center rounded-lg font-bold font-jp text-xl shrink-0">{pillar.char}</div>
                    <div>
                      <h4 className="text-lg font-bold mb-2 font-jp text-stone-900">{pillar.title}</h4>
      ...
      <section id="facility" className="py-24 bg-white scroll-mt-24 border-t border-stone-100">
        <div className="container mx-auto px-6 max-w-[1600px]">
          <div className="text-center mb-20">
            <h2 className="text-4xl font-black mb-4 font-jp tracking-tight text-stone-900">{t.facility.h2}</h2>
      ...
      <section id="exhibition" className="py-24 bg-stone-950 text-white scroll-mt-24 overflow-hidden border-b border-white/5">
        <div className="container mx-auto px-6 text-center mb-16">
          <h2 className="text-4xl font-black mb-4 font-jp tracking-tight">{t.exhibition.h2}</h2>
          <div className="w-20 h-1 bg-red-600 mx-auto mb-6"></div>
          <p className="text-stone-400 tracking-[0.3em] text-[10px] font-bold uppercase">{t.exhibition.subtitle}</p>
        </div>
        <div className="relative">
      ...
      <section id="courses" className="py-24 bg-stone-50 border-y border-stone-200 scroll-mt-24">
        <div className="container mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl font-black mb-4 font-jp tracking-tight text-stone-900">{t.courses.h2}</h2>
            <div className="w-20 h-1 bg-red-600 mx-auto mb-6"></div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-16 gap-8">
            <div className="max-w-3xl">
              <div className="text-red-600 font-bold tracking-[0.3em] text-[10px] mb-4 uppercase">{t.alumni.label}</div>
              <h2 className="text-4xl md:text-6xl font-black mb-8 font-jp leading-tight">
                <Nl2br text={t.alumni.h2} />
              </h2>
            </div>
          </div>
          <div className="relative">
          ...
          <div className="grid md:grid-cols-3 gap-12 mb-20">
            ...
            <div>
              ...
              <ul className="space-y-4 text-xs font-bold text-stone-500 uppercase tracking-widest">
                <li><button onClick={() => scrollToSection('courses')} className="hover:text-red-600 transition-colors uppercase">{t.nav.courses}</button></li>
                <li><button onClick={() => scrollToSection('exhibition')} className="hover:text-red-600 transition-colors uppercase">{t.nav.exhibition}</button></li>
                <li><button onClick={() => scrollToSection('instructors')} className="hover:text-red-600 transition-colors uppercase">{t.nav.instructors}</button></li>
